 ...
 * - itemFilter: 특정 상품명/카테고리명으로 필터링
 * - responseHint: 특정 속성만 응답 (sku, currentStock, optimalStock, minimumStock, stockout, status)
 */
async function queryInventory(
  ...
  if (itemFilter && itemFilter.length > 0) {
    ...
    if (filtered.length === 0) {
      return {
        actions,
        message: `"${itemFilter.join(', ')}"에 해당하는 재고 정보를 찾을 수 없습니다.${tabMessage}`,
    ...
    // responseHint에 따라 특정 속성만 응답
    if (responseHint && filtered.length > 0) {
      const list = filtered.map((i: any) => {
        switch (responseHint) {
          case 'sku':
            return `• ${i.product_name}: SKU ${i.sku || '없음'}`;
          case 'currentStock':
            return `• ${i.product_name}: 현재고 ${(i.current_stock || 0).toLocaleString()}개`;
          case 'optimalStock':
            return `• ${i.product_name}: 적정재고 ${(i.optimal_stock || 0).toLocaleString()}개`;
          case 'minimumStock':
            return `• ${i.product_name}: 최소재고 ${(i.minimum_stock || 0).toLocaleString()}개`;
          case 'stockout':
            return `• ${i.product_name}: 품절 예상 ${i.days_until_stockout != null ? `${i.days_until_stockout}일` : '산정 불가'}`;
          case 'status':
            return `• ${i.product_name}: ${statusLabel[i.stock_status] || i.stock_status}`;
          default:
            return formatInventoryItem(i, statusLabel);
        }
      }).join('\n');
...
/**
 * 정상 재고 조회 (RPC: get_inventory_status → normal 상태 필터)
 */
async function queryHealthyStock(
  supabase: SupabaseClient,
  storeId: string,
  dateRange: { startDate: string; endDate: string },
  pageContext?: PageContext,
...
/**
 * 카테고리별 재고 현황 조회 (RPC: get_inventory_status → 카테고리별 그룹화)
 */
async function queryInventoryCategory(
  supabase: SupabaseClient,
  storeId: string,
  dateRange: { startDate: string; endDate: string },
  pageContext?: PageContext,
...
/**
 * 상세 재고 현황 조회 (RPC: get_inventory_status → 전체 상품 목록)
 */
async function queryInventoryDetail(
  supabase: SupabaseClient,
  storeId: string,
  dateRange: { startDate: string; endDate: string },
  pageContext?: PageContext,
