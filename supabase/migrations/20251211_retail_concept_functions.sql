...
-- 1. 구역 전환 퍼널 계산
-- 각 구역의 방문자 수, 평균 체류 시간, 구매 수, 전환율을 계산
...
BEGIN
  ...
  WITH zone_data AS (
    ...
    WHERE ge.store_id = p_store_id
      AND oet.name IN ('Zone', 'zone', 'ZONE', 'Area', 'area')
  ),
  visit_data AS (
    SELECT
      gr.target_entity_id,
    ...
    WHERE gr.store_id = p_store_id
      AND ort.name IN ('VISITED', 'visited', 'ENTERED_ZONE', 'entered_zone')
      AND gr.created_at >= NOW() - (p_days || ' days')::INTERVAL
  ),
  purchase_data AS (
    SELECT DISTINCT
      gr.source_entity_id as customer_id
    FROM graph_relations gr
    JOIN ontology_relation_types ort ON gr.relation_type_id = ort.id
    WHERE gr.store_id = p_store_id
      AND ort.name IN ('PURCHASED', 'purchased', 'MADE_TRANSACTION', 'made_transaction')
      AND gr.created_at >= NOW() - (p_days || ' days')::INTERVAL
  )
  SELECT
...
BEGIN
  ...
  JOIN ontology_entity_types oet ON ge.entity_type_id = oet.id
  WHERE ge.store_id = p_store_id
    AND oet.name IN ('Transaction', 'transaction', 'TRANSACTION', 'Order', 'order');
  ...
  WITH transaction_items AS (
    ...
    JOIN ontology_entity_types oet ON ge.entity_type_id = oet.id
    WHERE gr.store_id = p_store_id
      AND ort.name IN ('CONTAINS', 'contains', 'HAS_ITEM', 'has_item', 'PURCHASED_PRODUCT', 'purchased_product')
      AND oet.name IN ('Product', 'product', 'PRODUCT', 'Item', 'item')
  )
  SELECT
...
BEGIN
  ...
  sales_data AS (
    ...
    JOIN ontology_relation_types ort ON gr.relation_type_id = ort.id
    WHERE gr.store_id = p_store_id
      AND ort.name IN ('CONTAINS', 'contains', 'HAS_ITEM', 'has_item', 'SOLD', 'sold')
      AND gr.created_at >= NOW() - (p_days || ' days')::INTERVAL
...
BEGIN
  ...
  SELECT jsonb_build_object(
    'total', COUNT(*),
    'by_type', jsonb_object_agg(COALESCE(oet.name, 'unknown'), cnt)
  )
  INTO entity_summary
  FROM (
    SELECT oet.name, COUNT(*) as cnt
    FROM graph_entities ge
    LEFT JOIN ontology_entity_types oet ON ge.entity_type_id = oet.id
    WHERE ge.store_id = p_store_id
    GROUP BY oet.name
  ) sub
  JOIN ontology_entity_types oet ON sub.name = oet.name;

  -- 6. 관계 요약
  SELECT jsonb_build_object(
    'total', COUNT(*),
    'by_type', jsonb_object_agg(COALESCE(ort.name, 'unknown'), cnt)
  )
  INTO relation_summary
  FROM (
    SELECT ort.name, COUNT(*) as cnt
    FROM graph_relations gr
    LEFT JOIN ontology_relation_types ort ON gr.relation_type_id = ort.id
    WHERE gr.store_id = p_store_id
    GROUP BY ort.name
  ) sub
  JOIN ontology_relation_types ort ON sub.name = ort.name;
  ...
  result := jsonb_build_object(
    ...
    'entity_summary', COALESCE(entity_summary, '{}'::jsonb),
    'relation_summary', COALESCE(relation_summary, '{}'::jsonb)
  );
