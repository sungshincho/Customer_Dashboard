    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-auto p-4 space-y-4">
        ...
        <div className="space-y-2">
          <label className="text-xs font-medium text-white/60">시뮬레이션 타입</label>
          <div className="grid grid-cols-2 gap-2">
            <button
              onClick={() => setSimulationType('realtime')}
            ...
            <button
              onClick={() => setSimulationType('prediction')}
              disabled={isAnyRunning}
              className={cn(
                "p-3 rounded-lg border text-left transition",
          ...
          {showAdvancedOptions && (
            <div className="p-3 pt-0 space-y-3 border-t border-white/10">
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                <input
                  type="checkbox"
                  checked={showCustomerLabels}
                  onChange={(e) => setShowCustomerLabels(e.target.checked)}
                  disabled={isAnyRunning}
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                <input
                  type="checkbox"
                  checked={showCongestionHeatmap}
                  onChange={(e) => setShowCongestionHeatmap(e.target.checked)}
                  disabled={isAnyRunning}
                  className="w-4 h-4 rounded bg-white/10"
                />
                <div>
        ...
        {isRealtimeRunning && simulationType === 'realtime' && (
        ...
        {/* AI 예측 결과 표시 */}
        {aiResult && simulationType === 'prediction' && (
          <div className="space-y-3">
            ...
            {/* AI 인사이트 */}
            {aiResult.ai_insights && aiResult.ai_insights.length > 0 && (
              <div className="p-3 bg-white/5 rounded-lg">
                ...
                <ul className="space-y-1.5">
                  {aiResult.ai_insights.slice(0, 3).map((insight, idx) => (
      ...
      <div className="p-4 border-t border-white/10 space-y-2">
        {!isAnyRunning ? (
          <Button
            ...
            disabled={!storeId}
            className={cn(
              "w-full py-3 font-medium text-white transition flex items-center justify-center gap-2",
              simulationType === 'realtime'
          ...
          <div className="flex gap-2">
            {/* 일시정지/재개 (실시간만) */}
            {simulationType === 'realtime' && isRealtimeRunning && (
              <Button
                onClick={handleTogglePause}
                className="flex-1 py-3 font-medium bg-yellow-600 hover:bg-yellow-700 text-white transition flex items-center justify-center gap-2"
              >
                {isPaused ? (
                  <>
                    <Play className="w-4 h-4" />
            ...
            <Button
              ...
              className={cn(
                "py-3 font-medium bg-red-600 hover:bg-red-700 text-white transition flex items-center justify-center gap-2",
                simulationType === 'realtime' && isRealtimeRunning ? "flex-1" : "w-full"
              )}
            >
