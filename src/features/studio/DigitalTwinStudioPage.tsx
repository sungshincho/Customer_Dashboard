          ...
          <div className="absolute inset-0 z-0">
            {loading ? <div className="flex items-center justify-center h-full">
              ...
              return null;
                ...
                {/* 시뮬레이션 결과 오버레이 - 🔧 FIX: 결과가 있으면 항상 표시 (viewMode 조건 제거) */}
                {sceneSimulation.state.results.layout && <LayoutOptimizationOverlay result={sceneSimulation.state.results.layout as any} showBefore={false} showAfter={false} showMoves={true} showProductMoves={true} showZoneHighlights={true} storeBounds={storeBounds} zonePositions={zonePositions} zoneSizes={zoneSizes} />}
                {sceneSimulation.state.results.flow && <FlowOptimizationOverlay result={sceneSimulation.state.results.flow as any} showPaths={true} showBottlenecks={true} showHeatmap={true} animatePaths={true} storeBounds={storeBounds} entrancePosition={entrancePosition} />}
                {sceneSimulation.state.results.congestion && <CongestionOverlay result={sceneSimulation.state.results.congestion as any} showHeatmap={true} showZoneMarkers={true} showCrowdAnimation={true} animateTimeProgress={false} />}
                {sceneSimulation.state.results.staffing && <StaffingOverlay result={sceneSimulation.state.results.staffing as any} showStaffMarkers={true} showCurrentPositions={true} showSuggestedPositions={true} showCoverageZones={true} showMovementPaths={true} animateMovement={true} />}
