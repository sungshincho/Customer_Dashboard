        ...
        <div className="digital-twin-studio relative w-full h-[calc(100vh-120px)] overflow-hidden bg-black rounded-lg">
          ...
          <div className="absolute inset-0 z-0">
            {loading ? <div className="flex items-center justify-center h-full">
              ...
              return null;
                ...
                {sceneSimulation.state.results.layout && viewMode !== 'as-is' && <LayoutOptimizationOverlay result={sceneSimulation.state.results.layout as any} showBefore={false} /* 🔧 FIX: Before 히트맵 비활성화 (빨간 오버레이 방지) */ showAfter={false} /* 🔧 FIX: After 히트맵도 비활성화 (깔끔한 뷰) */ showMoves={true} /* 가구 이동 화살표는 항상 표시 */ showProductMoves={true} /* 제품 이동 화살표도 항상 표시 */ showZoneHighlights={viewMode === 'to-be'} /* To-Be에서만 존 하이라이트 */ storeBounds={storeBounds} zonePositions={zonePositions} zoneSizes={zoneSizes} />}
                {sceneSimulation.state.results.flow && viewMode !== 'as-is' && <FlowOptimizationOverlay result={sceneSimulation.state.results.flow as any} showPaths={true} showBottlenecks={true} showHeatmap={true} animatePaths={true} storeBounds={storeBounds} entrancePosition={entrancePosition} />}
                {sceneSimulation.state.results.congestion && viewMode !== 'as-is' && <CongestionOverlay result={sceneSimulation.state.results.congestion as any} showHeatmap={true} showZoneMarkers={true} showCrowdAnimation={true} animateTimeProgress={false} />}
                {sceneSimulation.state.results.staffing && viewMode as string !== 'as-is' && <StaffingOverlay result={sceneSimulation.state.results.staffing as any} showStaffMarkers={true} showCurrentPositions={viewMode as string === 'split'} showSuggestedPositions={viewMode as string === 'to-be' || viewMode as string === 'split'} showCoverageZones={viewMode as string !== 'as-is'} showMovementPaths={viewMode as string === 'to-be' || viewMode as string === 'split'} animateMovement={true} />}
          ...
          <div className="absolute inset-0 z-10 pointer-events-none">
            ...
            <div className="absolute top-4 right-4 pointer-events-auto z-20">
              <ViewModeToggle mode={viewMode} onChange={setViewMode} hasOptimizationResults={!!sceneSimulation.state.results.layout || !!sceneSimulation.state.results.flow} />
            </div>
