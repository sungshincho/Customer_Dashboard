...
async function buildControlTowerStatusFallback(storeId: string): Promise<DataControlTowerStatus> {
  ...
  return {
    ...
    data_sources: {
      pos: { name: 'POS', description: '매출/거래 데이터', status: (posCount || 0) > 0 ? 'active' : 'inactive', last_sync: null },
      ...
      product: { name: 'ERP', description: '재고/상품 데이터', status: (productCount || 0) > 0 ? 'active' : 'inactive', last_sync: null },
    },
