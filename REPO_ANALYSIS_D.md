...
> 아래 부록들은 각 주제별 상세 참고 자료입니다. 본문 섹션 1~12에서 요약된 내용의 세부 사항을 담고 있습니다.

---

## 부록 A: 유틸리티 함수 & 데이터 아키텍처
...
### A.2 데이터 정규화 엔진 (`dataNormalizer.ts`)
...
### A.4 엔터프라이즈 스키마 (`enterpriseSchemas.ts`)
...
### A.5 의존성 그래프 (`dependencyGraph.ts`)
...
### A.6 역할 기반 접근 제어 (`rolePermissions.ts`)
...
## 부록 C: UI 디자인 시스템 & 피처 플래그

### C.1 Tailwind CSS 커스텀 설정
...
### C.4 피처 플래그 시스템 (`src/config/featureFlags.ts`)
