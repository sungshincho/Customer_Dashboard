# Digital Twin 3D 시스템 설정 가이드

**버전**: 1.1
**작성일**: 2025-12-16
**NEURALTWIN**: v8.3

---

## 📋 목차

1. [개요](#1-개요)
2. [전체 To-Do List](#2-전체-to-do-list)
3. [자동화 vs 수동 작업 구분](#3-자동화-vs-수동-작업-구분)
4. [SQL 실행 순서](#4-sql-실행-순서)
5. [MVP 테스트 가이드](#5-mvp-테스트-가이드)
6. [3D 모델 제작 스펙](#6-3d-모델-제작-스펙)
7. [Storage 업로드 가이드](#7-storage-업로드-가이드)
8. [테스트 시나리오](#8-테스트-시나리오)
9. [데이터 편집 가이드](#9-데이터-편집-가이드) ← 신규

---

## 1. 개요

이 문서는 NEURALTWIN Digital Twin 3D 시스템을 설정하고 MVP 테스트를 진행하기 위한 종합 가이드입니다.

### 시스템 구성요소

```
┌─────────────────────────────────────────────────────────────────────────┐
│                    NEURALTWIN Digital Twin System                       │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  [Database Layer]                                                       │
│  ├── stores (매장 기본 정보)                                            │
│  ├── zones_dim (구역 정의)                                              │
│  ├── furniture (가구 엔티티) ← 신규                                     │
│  ├── furniture_slots (슬롯 정의)                                        │
│  ├── products (상품 + display_type)                                    │
│  └── staff (직원 + avatar)                                             │
│                                                                         │
│  [3D Asset Layer]                                                       │
│  ├── Space Models (공간 GLB)                                           │
│  ├── Furniture Models (가구 GLB)                                       │
│  ├── Product Models (상품 GLB)                                         │
│  └── Avatar Models (아바타 GLB)                                        │
│                                                                         │
│  [Frontend Layer]                                                       │
│  ├── SceneViewer (3D 뷰어)                                             │
│  ├── PlacementEditor (배치 편집기)                                     │
│  ├── OptimizationPanel (AI 최적화)                                     │
│  └── ComparisonView (비교 뷰)                                          │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```

---

## 2. 전체 To-Do List

### Phase 1: Database 설정 (자동화 가능 ✅)

| # | 작업 | 파일 | 상태 |
|---|------|------|------|
| 1.1 | 기본 마이그레이션 실행 | 기존 migrations | ⬜ |
| 1.2 | furniture 테이블 생성 | `20251216_furniture_table.sql` | ⬜ |
| 1.3 | placement 테이블 생성 | `20251216_product_placement_and_avatars.sql` | ⬜ |
| 1.4 | display_type 컬럼 추가 | `20251216_display_type_and_slots.sql` | ⬜ |

### Phase 2: 시드 데이터 (자동화 가능 ✅)

| # | 작업 | 파일 | 상태 |
|---|------|------|------|
| 2.1 | 기본 데이터 시딩 | `NEURALTWIN_UNIFIED_SEED_v8.sql` | ⬜ |
| 2.2 | 상품/가구/슬롯 통합 | `NEURALTWIN_v8.3_unified_products_furniture.sql` | ⬜ |

### Phase 3: 3D 모델 제작 (수동 작업 🔴)

| # | 작업 | 파일명 | 상태 |
|---|------|--------|------|
| 3.1 | 공간 모델 | `store_simple_10x10.glb` | ⬜ |
| 3.2 | 행거 랙 모델 | `rack_hanger_simple.glb` | ⬜ |
| 3.3 | 선반 모델 | `shelf_simple.glb` | ⬜ |
| 3.4 | 테이블 모델 | `table_simple.glb` | ⬜ |
| 3.5 | 신발 랙 모델 | `rack_shoes_simple.glb` | ⬜ |
| 3.6 | 코트 모델 | `product_coat.glb` | ⬜ |
| 3.7 | 스웨터 모델 | `product_sweater.glb` | ⬜ |
| 3.8 | 신발 모델 | `product_shoes.glb` | ⬜ |
| 3.9 | 선물박스 모델 | `product_giftbox.glb` | ⬜ |
| 3.10 | 티셔츠팩 모델 | `product_tshirt_stack.glb` | ⬜ |
| 3.11 | 직원 아바타 | `avatar_staff.glb` | ⬜ |
| 3.12 | 고객 아바타 | `avatar_customer.glb` | ⬜ |

### Phase 4: Storage 연결 (수동 작업 🔴)

| # | 작업 | 상태 |
|---|------|------|
| 4.1 | Supabase Storage 버킷 생성 | ⬜ |
| 4.2 | GLB 파일 업로드 | ⬜ |
| 4.3 | `MVP_MODEL_URL_UPDATE.sql` 수정 | ⬜ |
| 4.4 | URL 연결 스크립트 실행 | ⬜ |

### Phase 5: 테스트 (수동 작업 🔴)

| # | 작업 | 상태 |
|---|------|------|
| 5.1 | 기본 렌더링 테스트 | ⬜ |
| 5.2 | 슬롯 스냅 테스트 | ⬜ |
| 5.3 | 호환성 체크 테스트 | ⬜ |
| 5.4 | AI 최적화 테스트 | ⬜ |

---

## 3. 자동화 vs 수동 작업 구분

### ✅ 자동화 가능 (Claude가 실행/생성)

```
┌─────────────────────────────────────────────────────────────────────────┐
│                        자동화 가능 작업                                  │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  📄 SQL 마이그레이션/시드 파일                                          │
│  ────────────────────────────────────────────────────────────────────   │
│  ✅ 20251216_furniture_table.sql        - 가구 테이블 생성              │
│  ✅ 20251216_product_placement_and_avatars.sql - 배치 시스템           │
│  ✅ 20251216_display_type_and_slots.sql - 디스플레이 타입              │
│  ✅ NEURALTWIN_UNIFIED_SEED_v8.sql      - 기본 데이터                  │
│  ✅ NEURALTWIN_v8.3_unified_products_furniture.sql - 통합 시드        │
│  ✅ MVP_MODEL_URL_UPDATE.sql            - URL 연결 템플릿              │
│                                                                         │
│  📦 프론트엔드 컴포넌트                                                 │
│  ────────────────────────────────────────────────────────────────────   │
│  ✅ SceneViewer.tsx                     - 3D 뷰어                      │
│  ✅ PlacementEditor.tsx                 - 배치 편집기                  │
│  ✅ OptimizationPanel.tsx               - AI 최적화 패널               │
│  ✅ ComparisonView.tsx                  - 비교 뷰                      │
│  ✅ useOptimization.ts                  - 최적화 훅                    │
│  ✅ usePlacement.ts                     - 배치 관리 훅                 │
│  ✅ useFurnitureSlots.ts               - 슬롯 관리 훅                  │
│                                                                         │
│  ⚙️ Edge Functions                                                     │
│  ────────────────────────────────────────────────────────────────────   │
│  ✅ generate-optimization/index.ts      - AI 최적화 생성               │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```

### 🔴 수동 작업 필요 (사용자가 직접)

```
┌─────────────────────────────────────────────────────────────────────────┐
│                        수동 작업 필요                                    │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  🎨 3D 모델 제작 (Blender 등)                                          │
│  ────────────────────────────────────────────────────────────────────   │
│  🔴 store_simple_10x10.glb    - 매장 공간 모델 (10m×10m)              │
│  🔴 rack_hanger_simple.glb    - 의류 행거 모델                         │
│  🔴 shelf_simple.glb          - 선반 진열대 모델                       │
│  🔴 table_simple.glb          - 테이블 모델                            │
│  🔴 rack_shoes_simple.glb     - 신발 랙 모델                           │
│  🔴 product_coat.glb          - 코트 모델 (hanging)                   │
│  🔴 product_sweater.glb       - 스웨터 모델 (folded)                  │
│  🔴 product_shoes.glb         - 신발 모델 (standing)                  │
│  🔴 product_giftbox.glb       - 선물박스 모델 (boxed)                 │
│  🔴 product_tshirt_stack.glb  - 티셔츠팩 모델 (stacked)               │
│  🔴 avatar_staff.glb          - 직원 아바타                            │
│  🔴 avatar_customer.glb       - 고객 아바타                            │
│                                                                         │
│  ☁️ Supabase Storage 작업                                              │
│  ────────────────────────────────────────────────────────────────────   │
│  🔴 Storage 버킷 생성 (3d-models)                                      │
│  🔴 GLB 파일 업로드                                                    │
│  🔴 Public URL 확인                                                    │
│                                                                         │
│  📝 URL 연결 (SQL 수정)                                                │
│  ────────────────────────────────────────────────────────────────────   │
│  🔴 MVP_MODEL_URL_UPDATE.sql 에서 v_storage_base URL 수정              │
│  🔴 실제 Storage URL로 교체                                            │
│                                                                         │
│  📏 슬롯 좌표 조정 (3D 모델 측정 후)                                    │
│  ────────────────────────────────────────────────────────────────────   │
│  ⚠️ 선택사항: 모델 실제 크기에 맞게 slot_position 미세 조정            │
│  ⚠️ Blender에서 가구 origin 기준 슬롯 위치 측정                        │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```

---

## 4. SQL 실행 순서

### 실행 명령어 (순서 중요!)

```bash
# ═══════════════════════════════════════════════════════════════════════
# STEP 1: 마이그레이션 실행
# ═══════════════════════════════════════════════════════════════════════

# 1-1. Furniture 테이블 생성
psql -h <DB_HOST> -U postgres -d postgres \
  -f supabase/migrations/20251216_furniture_table.sql

# 1-2. Product Placement 시스템
psql -h <DB_HOST> -U postgres -d postgres \
  -f supabase/migrations/20251216_product_placement_and_avatars.sql

# 1-3. Display Type & Slots
psql -h <DB_HOST> -U postgres -d postgres \
  -f supabase/migrations/20251216_display_type_and_slots.sql

# ═══════════════════════════════════════════════════════════════════════
# STEP 2: 시드 데이터 실행
# ═══════════════════════════════════════════════════════════════════════

# 2-1. 기본 데이터 (이미 실행된 경우 스킵)
psql -h <DB_HOST> -U postgres -d postgres \
  -f supabase/seeds/NEURALTWIN_UNIFIED_SEED_v8.sql

# 2-2. 통합 상품/가구/슬롯 데이터
psql -h <DB_HOST> -U postgres -d postgres \
  -f supabase/seeds/NEURALTWIN_v8.3_unified_products_furniture.sql

# ═══════════════════════════════════════════════════════════════════════
# STEP 3: 3D 모델 업로드 후 URL 연결
# ═══════════════════════════════════════════════════════════════════════

# 3-1. MVP_MODEL_URL_UPDATE.sql 수정 (v_storage_base URL 변경)
# 3-2. URL 연결 스크립트 실행
psql -h <DB_HOST> -U postgres -d postgres \
  -f supabase/seeds/MVP_MODEL_URL_UPDATE.sql
```

### Supabase Dashboard에서 실행

```sql
-- Supabase SQL Editor에서 순서대로 실행:

-- 1. 마이그레이션
-- (migrations 폴더의 파일들 복사 & 붙여넣기)

-- 2. 시드 데이터
-- (seeds 폴더의 파일들 복사 & 붙여넣기)

-- 3. URL 연결 (3D 모델 업로드 후)
-- MVP_MODEL_URL_UPDATE.sql 내용 수정 후 실행
```

---

## 5. MVP 테스트 가이드

### MVP 모델 구성 (12개)

```
┌─────────────────────────────────────────────────────────────────────────┐
│                         MVP 모델 목록                                    │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  [공간 - 1개]                                                           │
│  ├── store_simple_10x10.glb                                            │
│  │   • 크기: 10m × 10m × 3m                                            │
│  │   • 구성: 바닥 + 벽 4면                                              │
│  │   • 폴리곤: ~100                                                     │
│                                                                         │
│  [가구 - 4개]                                                           │
│  ├── rack_hanger_simple.glb (행거)                                     │
│  │   • 크기: 1.2m × 1.8m × 0.5m                                        │
│  │   • 슬롯: 10개 (H1~H10)                                              │
│  │   • 호환: hanging                                                   │
│  │                                                                      │
│  ├── shelf_simple.glb (선반)                                           │
│  │   • 크기: 1.0m × 1.8m × 0.4m                                        │
│  │   • 슬롯: 12개 (4단 × 3열)                                          │
│  │   • 호환: folded, boxed, standing                                   │
│  │                                                                      │
│  ├── table_simple.glb (테이블)                                         │
│  │   • 크기: 1.2m × 0.9m × 0.8m                                        │
│  │   • 슬롯: 6개 (2행 × 3열)                                           │
│  │   • 호환: folded, boxed, stacked                                    │
│  │                                                                      │
│  └── rack_shoes_simple.glb (신발랙)                                    │
│      • 크기: 1.2m × 1.5m × 0.4m                                        │
│      • 슬롯: 16개 (4단 × 4열)                                          │
│      • 호환: standing                                                   │
│                                                                         │
│  [상품 - 5개] (각 display_type 대표)                                    │
│  ├── product_coat.glb → hanging                                        │
│  ├── product_sweater.glb → folded                                      │
│  ├── product_shoes.glb → standing                                      │
│  ├── product_giftbox.glb → boxed                                       │
│  └── product_tshirt_stack.glb → stacked                                │
│                                                                         │
│  [아바타 - 2개] (선택사항)                                               │
│  ├── avatar_staff.glb                                                  │
│  └── avatar_customer.glb                                               │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```

### MVP 테스트 체크리스트

```
□ 테스트 1: 기본 렌더링
  □ 공간 모델 로딩 확인
  □ 가구 4종 배치 확인
  □ 상품 5개 슬롯 위치 렌더링
  □ 카메라 컨트롤 (회전/줌/패닝)

□ 테스트 2: 슬롯 스냅
  □ 상품 드래그 시 슬롯 하이라이트
  □ 호환 슬롯에만 스냅 확인
    - coat → hanger 슬롯 (O)
    - coat → shelf 슬롯 (X)
  □ 비호환 슬롯 빨간색 표시

□ 테스트 3: 배치 저장
  □ 상품 위치 변경 후 저장
  □ 페이지 새로고침 후 복원
  □ Undo/Redo 동작

□ 테스트 4: AI 최적화
  □ 최적화 생성 요청
  □ 추천 결과 패널 표시
  □ Before/After 비교 뷰
  □ 변경사항 적용/거절
```

---

## 6. 3D 모델 제작 스펙

### 공통 요구사항

| 항목 | 요구사항 |
|------|----------|
| 포맷 | GLB (glTF Binary) |
| 단위 | 미터 (1 unit = 1 meter) |
| 원점 | 모델 바닥 중앙 |
| Y축 | 위쪽 (Y-up) |
| 텍스처 | 선택사항 (MVP는 solid color 권장) |
| 최대 폴리곤 | 공간 1K, 가구 500, 상품 500, 아바타 1K |

### 가구별 슬롯 위치 정의

```
┌─────────────────────────────────────────────────────────────────────────┐
│  rack_hanger_simple.glb - 슬롯 위치 (가구 원점 기준)                    │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│     ┌────────────────────────────────┐                                 │
│     │  H1  H2  H3  H4  H5  H6  H7  H8  H9  H10  │  ← y=1.6m             │
│     │   ○   ○   ○   ○   ○   ○   ○   ○   ○   ○   │                       │
│     └────────────────────────────────┘                                 │
│     x: -0.5 ~ +0.5 (간격 0.11m)                                        │
│                                                                         │
│  slot_position 예시:                                                    │
│  H1: {"x": -0.5, "y": 1.6, "z": 0}                                     │
│  H2: {"x": -0.39, "y": 1.6, "z": 0}                                    │
│  ...                                                                    │
└─────────────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────────────┐
│  shelf_simple.glb - 슬롯 위치 (가구 원점 기준)                          │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│     ┌─────────────────┐                                                │
│     │ S4-1  S4-2  S4-3 │ ← y=1.4m                                      │
│     ├─────────────────┤                                                │
│     │ S3-1  S3-2  S3-3 │ ← y=1.0m                                      │
│     ├─────────────────┤                                                │
│     │ S2-1  S2-2  S2-3 │ ← y=0.6m                                      │
│     ├─────────────────┤                                                │
│     │ S1-1  S1-2  S1-3 │ ← y=0.2m                                      │
│     └─────────────────┘                                                │
│     x: -0.3 ~ +0.3 (간격 0.3m)                                         │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```

---

## 7. Storage 업로드 가이드

### Supabase Storage 설정

```bash
# 1. Supabase Dashboard > Storage > New Bucket
Bucket name: 3d-models
Public: Yes (체크)

# 2. 폴더 구조 생성
3d-models/
├── spaces/
│   └── store_simple_10x10.glb
├── furniture/
│   ├── rack_hanger_simple.glb
│   ├── shelf_simple.glb
│   ├── table_simple.glb
│   └── rack_shoes_simple.glb
├── products/
│   ├── product_coat.glb
│   ├── product_sweater.glb
│   ├── product_shoes.glb
│   ├── product_giftbox.glb
│   └── product_tshirt_stack.glb
└── avatars/
    ├── avatar_staff.glb
    └── avatar_customer.glb

# 3. Public URL 형식
https://<project-id>.supabase.co/storage/v1/object/public/3d-models/spaces/store_simple_10x10.glb
```

### URL 연결 (MVP_MODEL_URL_UPDATE.sql)

```sql
-- MVP_MODEL_URL_UPDATE.sql 파일을 열고 아래 부분 수정:

DO $$
DECLARE
  -- ⚠️ 실제 URL로 변경!
  v_storage_base TEXT := 'https://YOUR_PROJECT_ID.supabase.co/storage/v1/object/public/3d-models';
  ...
```

---

## 8. 테스트 시나리오

### 시나리오 1: 기본 렌더링 확인

```typescript
// DigitalTwinStudio 페이지에서:
1. SceneViewer 컴포넌트 렌더링 확인
2. 공간 모델 로딩 → 매장 바닥/벽 표시
3. 가구 모델 로딩 → 4종 가구 배치
4. 상품 모델 로딩 → 5개 MVP 상품 표시
5. 카메라 컨트롤 테스트
```

### 시나리오 2: 슬롯 호환성 테스트

```
1. 코트(hanging) 선택
2. 행거 슬롯으로 드래그 → 초록색 하이라이트 (호환)
3. 선반 슬롯으로 드래그 → 빨간색 하이라이트 (비호환)
4. 행거 슬롯에 드롭 → 자동 스냅
```

### 시나리오 3: AI 최적화 테스트

```
1. OptimizationPanel에서 "최적화 생성" 클릭
2. generate-optimization Edge Function 호출
3. 결과 표시 (추천 변경사항)
4. ComparisonView로 Before/After 비교
5. 변경사항 적용 또는 거절
```

---

## 📞 문제 해결

### 자주 발생하는 문제

| 문제 | 해결방법 |
|------|----------|
| 모델이 로딩되지 않음 | Storage URL 확인, CORS 설정 확인 |
| 슬롯 위치가 맞지 않음 | Blender에서 모델 원점 확인, slot_position 조정 |
| 상품이 슬롯에 스냅되지 않음 | compatible_display_types 배열 확인 |
| AI 최적화 실패 | Edge Function 로그 확인, API 키 설정 확인 |

---

## 9. 데이터 편집 가이드

이 섹션에서는 가구 위치, 슬롯 정보, 상품 배치 등을 직접 수정할 때 어떤 파일의 어느 부분을 편집해야 하는지 설명합니다.

### 9.1 편집 파일 위치 요약

| 데이터 | 파일 | 섹션 |
|--------|------|------|
| **가구 위치/회전/크기** | `NEURALTWIN_v8.3_unified_products_furniture.sql` | STEP 2 |
| **슬롯 개수 및 위치** | `NEURALTWIN_v8.3_unified_products_furniture.sql` | STEP 3 |
| **상품-슬롯 배치** | `MVP_MODEL_URL_UPDATE.sql` | MVP Product-Slot Assignment |
| **3D 모델 URL** | `MVP_MODEL_URL_UPDATE.sql` | 상단 v_storage_base |

---

### 9.2 가구 위치 편집

**파일**: `supabase/seeds/NEURALTWIN_v8.3_unified_products_furniture.sql`

**섹션**: STEP 2: Create Furniture Entities (약 100번째 줄)

```sql
-- 가구 위치 편집 예시
INSERT INTO furniture (
  id, store_id, user_id, org_id, zone_id,
  furniture_code, furniture_name, furniture_type,
  width, height, depth,
  position, rotation, scale,        -- ← 여기서 편집
  movable, is_active
) VALUES
-- 의류 행거 #1
('b0000001-0000-0000-0000-000000000001'::UUID, v_store_id, v_user_id, v_org_id, v_zone_clothing,
 'RACK-001', '의류 행거 (더블) #1', 'clothing_rack_double',
 1.2, 1.8, 0.5,                     -- width, height, depth (미터)
 '{"x":-6,"y":0,"z":2}'::jsonb,     -- ← position (월드 좌표)
 '{"x":0,"y":0,"z":0}'::jsonb,      -- ← rotation (각도)
 '{"x":1,"y":1,"z":1}'::jsonb,      -- ← scale (배율)
 true, true),

-- 선반형 진열대 #1
('b0000011-0000-0000-0000-000000000011'::UUID, v_store_id, v_user_id, v_org_id, v_zone_clothing,
 'SHELF-001', '선반형 진열대 #1', 'shelf_display',
 1.0, 1.8, 0.4,
 '{"x":-3,"y":0,"z":3}'::jsonb,     -- ← 이 좌표 수정
 '{"x":0,"y":0,"z":0}'::jsonb,
 '{"x":1,"y":1,"z":1}'::jsonb,
 true, true),
...
```

**좌표 설명**:
- `position.x`: 좌우 위치 (음수: 왼쪽, 양수: 오른쪽)
- `position.y`: 높이 (바닥 = 0)
- `position.z`: 앞뒤 위치 (음수: 뒤쪽, 양수: 앞쪽)

---

### 9.3 슬롯 개수 및 위치 편집

**파일**: `supabase/seeds/NEURALTWIN_v8.3_unified_products_furniture.sql`

**섹션**: STEP 3: Create Furniture Slots (약 150번째 줄)

```sql
-- 행거 슬롯 편집 예시 (RACK-001)
INSERT INTO furniture_slots (
  furniture_id, furniture_type, slot_id, slot_type,
  slot_position, slot_rotation,          -- ← 슬롯 위치
  compatible_display_types,
  max_product_width, max_product_height, max_product_depth,
  store_id
)
SELECT
  'b0000001-0000-0000-0000-000000000001'::uuid,  -- furniture_id
  'clothing_rack_double',
  'H' || row_num,                        -- slot_id: H1, H2, H3...
  'hanger',
  jsonb_build_object(
    'x', -0.5 + (row_num - 1) * 0.11,    -- ← X 위치 (간격 0.11m)
    'y', 1.6,                             -- ← Y 높이 (1.6m)
    'z', 0                                -- ← Z 위치
  ),
  '{"x":0,"y":0,"z":0}'::jsonb,
  ARRAY['hanging'],                       -- ← 호환 display_type
  0.6, 1.2, 0.3,                         -- max 크기 제한
  v_store_id
FROM generate_series(1, 10) AS row_num;  -- ← 슬롯 개수 (10개)
```

#### 슬롯 개수 변경

```sql
-- 10개 → 8개로 변경
FROM generate_series(1, 8) AS row_num;
```

#### 슬롯 간격 변경

```sql
-- 간격 0.11m → 0.15m로 변경
'x', -0.5 + (row_num - 1) * 0.15,
```

#### 슬롯 높이 변경

```sql
-- 높이 1.6m → 1.8m로 변경
'y', 1.8,
```

---

### 9.4 선반/테이블 슬롯 (2D 배열)

```sql
-- 선반 슬롯 (4단 × 3열 = 12개)
SELECT
  'b0000011-0000-0000-0000-000000000011'::uuid,
  'shelf_display',
  'S' || shelf_level || '-' || col_num,  -- S1-1, S1-2, S2-1...
  'shelf',
  jsonb_build_object(
    'x', -0.3 + (col_num - 1) * 0.3,     -- ← 열 위치 (간격 0.3m)
    'y', 0.2 + (shelf_level - 1) * 0.4,  -- ← 단 높이 (간격 0.4m)
    'z', 0
  ),
  ...
FROM
  generate_series(1, 4) AS shelf_level,   -- ← 단 수 (4단)
  generate_series(1, 3) AS col_num;       -- ← 열 수 (3열)
```

---

### 9.5 상품-슬롯 배치 편집

**파일**: `supabase/seeds/MVP_MODEL_URL_UPDATE.sql`

**섹션**: MVP Product-Slot Assignment (약 100번째 줄)

```sql
-- 상품을 특정 가구의 특정 슬롯에 배치

-- 코트 → RACK-001의 H1 슬롯
UPDATE products SET
  initial_furniture_id = 'b0000001-0000-0000-0000-000000000001'::UUID,  -- ← 가구 ID
  slot_id = 'H1',                                                        -- ← 슬롯 ID
  model_3d_position = '{"x":-6,"y":1.6,"z":2}'::jsonb                   -- ← 월드 좌표
WHERE sku = 'SKU-OUT-001';

-- 슬롯 점유 상태 업데이트 (필수)
UPDATE furniture_slots SET
  is_occupied = true,
  occupied_by_product_id = 'f0000001-0000-0000-0000-000000000000'::UUID
WHERE furniture_id = 'b0000001-0000-0000-0000-000000000001'::UUID
  AND slot_id = 'H1';
```

#### 상품 월드 좌표 계산

```
상품 월드 좌표 = 가구 position + 슬롯 slot_position

예시:
├── 가구 position:      {"x":-6,   "y":0,   "z":2}
├── 슬롯 slot_position: {"x":-0.5, "y":1.6, "z":0}
└── 상품 월드 좌표:     {"x":-6.5, "y":1.6, "z":2}
```

---

### 9.6 호환성 매핑 (참고)

| slot_type | compatible_display_types | 대상 상품 예시 |
|-----------|--------------------------|----------------|
| `hanger` | `['hanging']` | 코트, 자켓, 셔츠, 팬츠 |
| `shelf` | `['folded', 'boxed', 'standing']` | 접힌 옷, 박스, 신발 |
| `table` | `['folded', 'boxed', 'stacked']` | 접힌 옷, 선물박스, 적층상품 |
| `rack` | `['standing']` | 신발, 가방 |

---

### 9.7 ID 참조표

#### 가구 ID (furniture)

| furniture_code | furniture_id | 타입 |
|----------------|--------------|------|
| RACK-001 | `b0000001-...-000000000001` | clothing_rack_double |
| RACK-002 | `b0000002-...-000000000002` | clothing_rack_double |
| RACK-003 | `b0000003-...-000000000003` | clothing_rack_double |
| RACK-004 | `b0000004-...-000000000004` | clothing_rack_double |
| SHELF-001 | `b0000011-...-000000000011` | shelf_display |
| SHELF-002 | `b0000012-...-000000000012` | shelf_display |
| TABLE-001 | `b0000021-...-000000000021` | table_display |
| TABLE-002 | `b0000022-...-000000000022` | table_display |
| SHOE-001 | `b0000031-...-000000000031` | shoe_rack |
| SHOE-002 | `b0000032-...-000000000032` | shoe_rack |
| SHOWCASE-001 | `b0000041-...-000000000041` | glass_showcase |
| SHOWCASE-002 | `b0000042-...-000000000042` | glass_showcase |

#### 상품 ID (products)

| SKU | product_id | display_type | MVP 모델 |
|-----|------------|--------------|----------|
| SKU-OUT-001 | `f0000001-...-000000000000` | hanging | product_coat.glb |
| SKU-UND-001 | `f0000020-...-000000000000` | folded | product_sweater.glb |
| SKU-SHO-001 | `f0000010-...-000000000000` | standing | product_shoes.glb |
| SKU-GFT-001 | `f0000024-...-000000000000` | boxed | product_giftbox.glb |
| SKU-TSK-001 | `f0000025-...-000000000000` | stacked | product_tshirt_stack.glb |

---

### 9.8 편집 워크플로우

```
┌─────────────────────────────────────────────────────────────────────────┐
│                      데이터 편집 워크플로우                               │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  1️⃣ 3D 모델 제작 후 실제 크기 측정                                       │
│     └── Blender에서 가구 모델의 슬롯 위치 측정                          │
│                                                                         │
│  2️⃣ NEURALTWIN_v8.3_unified_products_furniture.sql 편집                │
│     ├── STEP 2: 가구 position 수정 (월드 좌표)                         │
│     └── STEP 3: 슬롯 slot_position 수정 (상대 좌표)                    │
│                                                                         │
│  3️⃣ MVP_MODEL_URL_UPDATE.sql 편집                                      │
│     ├── v_storage_base URL 수정                                        │
│     └── 상품-슬롯 배치 수정                                             │
│                                                                         │
│  4️⃣ SQL 재실행                                                          │
│     ├── v8.3 시드 재실행 (가구/슬롯 업데이트)                          │
│     └── MVP URL 스크립트 실행 (URL/배치 적용)                          │
│                                                                         │
│  5️⃣ 프론트엔드 테스트                                                   │
│     └── SceneViewer에서 위치 확인                                       │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```

---

*문서 버전: 1.1*
*최종 업데이트: 2025-12-16*
